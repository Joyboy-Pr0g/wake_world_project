...
def main():
    ...
    if USE_SMOTE:
        ...
        if k < 1:
            ...
            smote = SMOTE(sampling_strategy="minority", random_state=42, k_neighbors=k)
    ...
    for thr in np.arange(0.15, 0.71, 0.01):
        ...
        if wr >= 0.75 and fp_ < best_fp:
            best_fp, best_thr, best_wr = fp_, float(thr), wr
        elif wr >= 0.75 and fp_ == best_fp and wr > best_wr:
            best_thr, best_wr = float(thr), wr

    if best_fp < int(1e9):
        optimal_threshold = best_thr
        print(f"\nOptimal threshold (val, min FP s.t. wr>=75%): {optimal_threshold:.3f} (val_FP={best_fp}, val_wr={best_wr:.3f})")
    else:
    ...
    if fp_count > 0:
        ...
        with open("hard_negatives.txt", "w", encoding="utf-8") as f:
            ...
            for p in fp_paths:
                f.write(str(p) + "\n")
        print(f"Hard negative mining: {fp_count} FP -> {len(fp_paths)} files -> hard_negatives.txt")
